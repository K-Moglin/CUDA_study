...
// CPU naive attention (single-head) for correctness
static void cpu_naive_attention(
  ...
  float inv_sqrt_d = 1.f / std::sqrt((float)d);
  ...
  for (int i = 0; i < N; ++i) {
    ...
    float l = 0.f;
    for (int j = 0; j < N; ++j) if (std::isfinite(scores[j])) l += std::exp(scores[j] - m);
...
int main(int argc, char** argv) {
  ...
  if (!(Bc == 64 || Bc == 128 || Bc == 256)) {
    printf("Use Bc=64/128/256 for this demo.\n");
    return 0;
  }
  ...
  auto launch = [&](){
    if (Bc == 64) {
      fa2_alg1_forward_cute_rowwise<64, BLOCK_THREADS><<<grid, block>>>(dQ,dK,dV,dO,dL,N,d,causal);
    } else if (Bc == 128) {
      fa2_alg1_forward_cute_rowwise<128, BLOCK_THREADS><<<grid, block>>>(dQ,dK,dV,dO,dL,N,d,causal);
    } else {
      fa2_alg1_forward_cute_rowwise<256, BLOCK_THREADS><<<grid, block>>>(dQ,dK,dV,dO,dL,N,d,causal);
    }
  };
  ...
  cudaEvent_t st, ed;
  ck(cudaEventCreate(&st), "event");
  ck(cudaEventCreate(&ed), "event");

  ck(cudaEventRecord(st), "record st");
  for (int it = 0; it < iters; ++it) launch();
  ck(cudaEventRecord(ed), "record ed");
  ck(cudaEventSynchronize(ed), "sync ed");

  float ms = 0.f;
  ck(cudaEventElapsedTime(&ms, st, ed), "elapsed");
  ms /= iters;
  ...
  for (int i=0;i<N*d;++i){
    ...
    float denom=std::max(1e-6f,fabsf(b));
    max_rel=std::max(max_rel,ae/denom);
  }
  float max_L=0.f;
  for (int i=0;i<N;++i) max_L=std::max(max_L,(float)fabs(hL[i]-refL[i]));

  double flops = 4.0 * (double)N * (double)N * (double)d; // rough: QK + PV
  double gflops = flops / (ms*1e-3) / 1e9;

  printf("FA2 Alg1 (CuTe tensor-view) CUDA test\n");
  printf("N=%d d=%d Bc=%d causal=%d iters=%d\n",N,d,Bc,causal,iters);
  printf("time=%.3f ms  approx GFLOPs=%.2f\n",ms,gflops);
  printf("max |O_gpu-O_cpu|=%.3e  max rel=%.3e  max |L_gpu-L_cpu|=%.3e\n",max_abs,max_rel,max_L);
