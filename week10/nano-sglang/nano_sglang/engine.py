...
class Engine:
    ...
    def decode_batch(self, sequences: list[Sequence], sampling_params: SamplingParams) -> list[int]:
        ...
        for layer_idx in range(self.model.num_layers):
            ...
            for seq in sequences:
                layer_cache = seq.past_key_values.layers[layer_idx]
                k = layer_cache.keys
            ...
            # Rebuild a cache whose batch dimension contains all running sequences.
        ...
        for i, seq in enumerate(sequences):
            ...
            for layer_idx in range(self.model.num_layers):
                layer_cache = new_cache.layers[layer_idx]
                k = layer_cache.keys[i:i+1, :, pad:pad + real_len, :]
                v = layer_cache.values[i:i+1, :, pad:pad + real_len, :]
                per_seq_cache.update(k.clone(), v.clone(), layer_idx)
    ...
    def generate(self, prompt: str, sampling_params: SamplingParams = None) -> str:
        ...
        # Stop early if prefill already produced EOS or hit the token budget.
        while (
            seq.output_token_ids[-1] != self.tokenizer.eos_token_id
            and seq.num_generated < seq.max_tokens
        ):
