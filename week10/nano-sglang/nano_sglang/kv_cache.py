...
class KVCache:
    ...
    def update(self, layer_idx: int, batch_idx: int,
        ...
        # Write the new KV slice into the reserved contiguous region.
    ...
    def get(self, layer_idx: int, batch_idx: int, seq_len: int):
        ...
        # Return a single-sequence view with the batch dimension preserved.
        key = self.keys[layer_idx][batch_idx:batch_idx + 1, :, :seq_len, :]
        value = self.values[layer_idx][batch_idx:batch_idx + 1, :, :seq_len, :]
        return key, value
