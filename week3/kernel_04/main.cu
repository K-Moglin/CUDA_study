...
int main(int argc, char** argv) {
  ...
  // Kernel4 params (must satisfy BM==BN and BM*BK == blockDim.x)
  constexpr int BM = 64;
  constexpr int BN = 64;
  constexpr int BK = 8;
  constexpr int TM = 8;
  ...
  dim3 grid(CEIL_DIV(N, BN), CEIL_DIV(M, BM), 1);  // (x=cols, y=rows) to match kernel mapping
  dim3 block(BM * BK, 1, 1);                       // 512 threads

  // warmup
  sgemm1DBlocktiling<BM, BN, BK, TM><<<grid, block>>>(M, N, K, alpha, dA, dB, beta, dC);
  ...
  std::cout << "Kernel4 (repo-style 1D blocktiling)"
