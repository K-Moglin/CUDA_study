...
app = modal.App("kernel_10_warptiling")
...
def run(m: int = 4096, n: int = 4096, k: int = 4096, iters: int = 50):
    ...
    subprocess.run(
        ["bash", "-lc", "ls -la && wc -c main.cu && head -n 5 main.cu"],
        check=True,
    )

    subprocess.run(
        ["nvcc", "-O3", "-std=c++17", "-arch=sm_90", "main.cu", "-o", "k10"],
        check=True,
    )
    subprocess.run(["./k10", str(m), str(n), str(k), str(iters)], check=True)
