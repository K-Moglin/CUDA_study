...
int main(int argc, char** argv) {
  ...
  dim3 grid(ceil_div(M, BLOCKSIZE), ceil_div(N, BLOCKSIZE), 1);
  // blockDim becomes 1D but keeps the same number of threads: 32*32 = 1024
  dim3 block(BLOCKSIZE * BLOCKSIZE, 1, 1);

  // Warmup
  sgemm_coalescing<<<grid, block>>>(M, N, K, alpha, dA, dB, beta, dC);
